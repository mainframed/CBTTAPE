...
static const char undocumented_msgÝ¨ = N_("This switch lacks documentation");
...
find_opt (const char *input, int lang_mask)
{
  ...
  int comp;
  size_t good, goodlen = 0;

  good = cl_options_count;
  match_wrong_lang = cl_options_count;

  /* find the longest match */
  for (x = 0; x < cl_options_count; x++)
  {
      const struct cl_option *opt = &cl_optionsÝx¨;

      /* Is this switch a prefix of the input?  */
      if (strncmp (input, opt->opt_text + 1, opt->opt_len) == 0)
      {
          if ((opt->flags & lang_mask)
              && (inputÝopt->opt_len¨ == '\0' || (opt->flags & CL_JOINED)))
          {
              if (opt->opt_len > goodlen)
              {
                  good = x;
                  goodlen = opt->opt_len;
              }
          }
          else if (match_wrong_lang == cl_options_count)
          {
              match_wrong_lang = x;
          }
      }
  }
  if (good == cl_options_count)
  {
      good = match_wrong_lang;
  }
  return (good);
}
...
find_opt (const char *input, int lang_mask)
{
  ...
  /* Find mn such this lexicographical inequality holds:
     cl_optionsÝmn¨ <= input < cl_optionsÝmn + 1¨.  */
  while (mx - mn > 1)
    {
      md = (mn + mx) / 2;
      opt_len = cl_optionsÝmd¨.opt_len;
      comp = strncmp (input, cl_optionsÝmd¨.opt_text + 1, opt_len);
  ...
  do
    {
      const struct cl_option *opt = &cl_optionsÝmn¨;
...
write_langs (unsigned int mask)
{
  ...
  for (n = 0; (lang_name = lang_namesÝn¨) != 0; n++)
    if (mask & (1U << n))
...
handle_option (const char **argv, unsigned int lang_mask)
{
  ...
  if (option->flags & CL_JOINED)
    {
	 ...
	 argument to be persistent until the program exits.  */
      arg = argvÝ0¨ + cl_optionsÝopt_index¨.opt_len + 1;
      if (!value)
	arg += strlen ("no-");
  ...
  if (!(option->flags & (lang_mask | CL_COMMON)))
    {
      complain_wrong_lang (argvÝ0¨, option, lang_mask);
      goto done;
    }
...
{
  num_in_fnames++;
  in_fnames = xrealloc (in_fnames, num_in_fnames * sizeof (in_fnamesÝ0¨));
  in_fnamesÝnum_in_fnames - 1¨ = filename;
}
...
decode_options (unsigned int argc, const char **argv)
{
  ...
  for (i = 1; i < argc; i++)
      ...
      if (!strcmp (argvÝi¨, "-O"))
	{
      ...
      else if (argvÝi¨Ý0¨ == '-' && argvÝi¨Ý1¨ == 'O')
	{
	  /* Handle -Os, -O2, -O3, -O69, ...  */
	  const char *p = &argvÝi¨Ý2¨;
...
print_param_help (void)
{
  ...
  for (i = 0; i < LAST_PARAM; i++)
    {
      const char *help = compiler_paramsÝi¨.help;
      const char *param = compiler_paramsÝi¨.option;
...
print_filtered_help (unsigned int flag)
{
  ...
  if (flag == CL_COMMON)
	...
	{
	  if ((cl_optionsÝi¨.flags & filter) != flag)
	    continue;

	  /* Skip help for internal switches.  */
	  if (cl_optionsÝi¨.flags & CL_UNDOCUMENTED)
	    continue;
	  ...
	  if (printedÝi¨)
	    {
	      duplicates = true;
	      indent = print_switch (cl_optionsÝi¨.opt_text, indent);
	    }
	}
      ...
      if (room < len)
	{
	  ...
	  for (i = 0; helpÝi¨; i++)
		...
		break;
	      if (helpÝi¨ == ' ')
		len = i;
	      else if ((helpÝi¨ == '-' || helpÝi¨ == '/')
		       && helpÝi + 1¨ != ' '
		       && i > 0 && ISALPHA (helpÝi - 1¨))
		len = i + 1;
	    }
	}
